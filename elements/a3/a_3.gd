...
func _on_shot_timer_timeout() -> void:
		shot()
		shotTimer.wait_time = randf_range(4.0, 5.5)
		shotTimer.start()
...
func shot():
	...
	if died == false:
		...
		ATween.tween_property(self, "position", Vector2(0.0, -offpos), 0.10).as_relative().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		await ATween.finished
		ATween = create_tween().set_parallel(true)
		ATween.tween_property(self, "position", Vector2(0.0, offpos), 0.5).as_relative().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)

...
func _ready() -> void:
	...
	if is_instance_valid(target):
		add_point(Vector2(target.global_position.x, target.global_position.y + yoffset))
...
func _process(delta: float) -> void:
	...
	if time_since_last_tick >= tick_rate:
		...
		add_point(Vector2(current_pos.x, current_pos.y + yoffset))
		points_lifetime.append(0.0)
	if get_point_count() > 0:
		set_point_position(get_point_count() - 1, current_pos)
	var i = 0
